...
- 模型请求使用独立的工作上下文：工具结果按类型预算并对重复内容去重，再按完整 user turn 从旧到新裁剪；工具调用与结果不会拆开，即使最新 turn 本身超预算也会保留。完整 session 历史单独保存，不因本次上下文裁剪而丢失。

## 上下文管理

- `AgentConfig.max_context_tokens` 表示近似的模型总上下文预算，`max_output_tokens` 为模型回复预留空间；实际历史输入预算为两者之差。标准库实现使用保守的字符/token 估算，不依赖特定 tokenizer。
- `read_file`、`search_files`、命令和测试输出在进入工作上下文前会限制长度，保留开头、结尾和省略标记；内容完全相同的重复 tool result 只保留引用提示。完整原始消息仍保存在 session 中，便于重启和审计。
- 因预算被移出的旧任务不会简单丢弃：本地从完整消息中确定性提取用户任务、涉及文件、验证状态、错误和完成摘要，生成短的 `deterministic_task_summary` 系统消息。摘要不调用模型，可重复生成，也不写回原始 session 历史。
